# Consolidate Terminal Agent into `scripts/gt.js` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Integrate the full reverse terminal agent daemon directly into `scripts/gt.js`, delete legacy script files (`scripts/terminal-agent.js` and `scripts/terminal-exec.js`), update `package.json` scripts, and migrate all unit/integration tests to reference `scripts/gt.js`.

**Architecture:**
1. `scripts/gt.js`: Incorporate `TaskManager`, PTY spawning (`node-pty`), File RPC (`handleFileRpc`), WebSocket tunnel connection (`connect`), keepalive/heartbeat, 12-hex Container ID generation, auto-naming (`<hostname>-<4hex>`), and 4009 conflict rejection directly into `scripts/gt.js`.
2. Clean Script Deprecation: Physically delete `scripts/terminal-agent.js` and `scripts/terminal-exec.js`.
3. `package.json` & Aliases:
   - `"gt": "node scripts/gt.js"`
   - `"terminal-agent": "node scripts/gt.js agent"`
   - `"terminal-exec": "node scripts/gt.js exec"`
4. Test Suite Migration: Update all test files in `tests/` to import/execute `scripts/gt.js`.

**Tech Stack:** Node.js, WebSocket (`ws`), Node-PTY, Express, Jest.

## Global Constraints
- **Zero Orphaned Legacy Scripts**: `scripts/terminal-agent.js` and `scripts/terminal-exec.js` must be deleted.
- **100% Behavioral Parity**: All PTY, RPC, TaskManager, auto-naming, and conflict rejection behaviors must work identically via `gt agent`.
- **Full Test Suite Success**: All 117+ test suites in `tests/` must pass cleanly.

---

### Task 1: Integrate Agent Daemon Directly into `scripts/gt.js`

**Files:**
- Modify: `scripts/gt.js`
- Test: `tests/gtCli.test.ts`, `tests/terminalAgentConflict.test.ts`
...
```typescript
import { TaskManager, parseControlMessage, resolveWebSocketUrl } from '../scripts/gt.js';
...
git commit -m "feat(gt): embed full agent daemon into scripts/gt.js"
```

---

### Task 2: Remove Legacy Files & Migrate Test Suites to `gt.js`

**Files:**
- Delete: `scripts/terminal-agent.js`
- Delete: `scripts/terminal-exec.js` (if present)
- Modify: `tests/terminalAgentConflict.test.ts`
- Modify: `tests/terminalAgentKeepalive.test.ts`
- Modify: `tests/terminalAgent.test.ts`
- Modify: `tests/terminalAgentCommandExec.test.ts`
- Modify: `tests/terminalTaskManager.test.ts`
- Modify: `tests/terminalExecCli.test.ts`
- Modify: `package.json`

**Interfaces:**
- Produces:
  - Clean filesystem with zero legacy standalone scripts.
...
1. In `tests/terminalAgentConflict.test.ts`:
   ...
   - In `execFile('node', [agentScript, 'agent', ...])`, pass `'agent'` as first argument.
2. In `tests/terminalAgentKeepalive.test.ts`:
   - Change `require('../scripts/terminal-agent.js')` to `require('../scripts/gt.js')`.
3. In `tests/terminalAgent.test.ts`:
   - Change `const agentPath = path.resolve(__dirname, '../scripts/terminal-agent.js');` to `path.resolve(__dirname, '../scripts/gt.js');`
   - Update script expectations to verify `scripts/gt.js`.
4. In `tests/terminalAgentCommandExec.test.ts`:
   - Change `require('../scripts/terminal-agent.js')` to `require('../scripts/gt.js')`.
5. In `tests/terminalTaskManager.test.ts`:
   - Change `require('../scripts/terminal-agent.js')` to `require('../scripts/gt.js')`.
6. In `tests/terminalExecCli.test.ts`:
...
```json
  ...
  "scripts": {
    ...
    "terminal-agent": "node scripts/gt.js agent",
    "terminal-exec": "node scripts/gt.js exec",
    "build:agent": "cd agent-rs && cargo build --release",
    "agent:rs": "./agent-rs/target/release/gt agent"
  }
```
...
git add scripts/gt.js tests/ package.json
git commit -m "refactor(terminal): remove legacy script files and migrate all tests to gt.js"
```

---

### Task 3: Documentation Update & Final Regression
...
```bash
git add CLAUDE.md
git commit -m "docs: finalize gt consolidation in CLAUDE.md and verify production build"
```
