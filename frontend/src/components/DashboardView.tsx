...
export default function DashboardView({ adminKey, onOpenSettings }: { adminKey: string; onOpenSettings?: () => void }) {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      {/* Read-Only Settings View with Trigger Button */}
      {status?.config && (
        <div className="bg-slate-800/80 border border-slate-700/60 rounded-xl p-6 shadow-lg space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-700/60">
            <div>
              <h3 className="text-md font-bold text-slate-100">Active Runtime Environment</h3>
              <p className="text-xs text-slate-400 mt-0.5">Live parameters currently governing API translations</p>
            </div>
            {onOpenSettings && (
              <button
                onClick={onOpenSettings}
                className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 rounded-xl text-xs font-bold text-white transition-colors shadow flex items-center space-x-1.5"
              >
                <span>⚙️</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-mono">
            <div className="bg-slate-900/80 p-3.5 rounded-xl border border-slate-800">
              ...
              <span className={`px-2 py-0.5 rounded font-bold text-[11px] ${status.config.systemRoleToInstruction ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-slate-800 text-slate-400'}`}>
          ...
          {status.config.customSystemInstruction && (
            <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800 font-mono text-xs">
              <span className="text-slate-400 block mb-1 text-[10px] uppercase font-bold">CUSTOM_SYSTEM_INSTRUCTION</span>
              <div className="text-slate-300 whitespace-pre-wrap max-h-32 overflow-y-auto leading-relaxed">
                {status.config.customSystemInstruction}
              </div>
            </div>
          )}
